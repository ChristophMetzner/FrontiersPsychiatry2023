...
# ScZ parameter changes (glutamate here here)
Jie_max = [x*(2.6)for x in [1.0, 0.95, 0.9, 0.85, 0.8, 0.75, 0.7, 0.65, 0.6]] # default value 2.6
...
scz_time_series = np.zeros((len(seeds),len(Jie_max), 80,fmrisize-j))
for k,Jie in enumerate(Jie_max):
    print(k)
    for i,seed in enumerate(seeds):
        ...
        aln.params['Jie_max']=Jie
        aln.params['seed']=seed
        aln.run(bold=True)
        #print(j)
        scz_time_series[i,k,:,:] = aln.BOLD.BOLD[:, j:]

np.save('Results/Aln_150s_40seeds_ScZ_changes_Jie.npy',scz_time_series)
